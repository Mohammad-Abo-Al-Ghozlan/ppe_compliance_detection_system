...
def run_hpo(config: PipelineConfig, model_config: ModelConfig, project_dir: Path) -> dict[str, Any]:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trial_model_cfg = ModelConfig(name=f"{model_config.name}_trial_{trial.number}", weights=model_config.weights)
        
        try:
            detector_class = DetectorRegistry.get(model_config.name.split('_')[0] if '_' in model_config.name else 'yolo11n')
...
def train_pipeline(
    ...
    if not models_to_train:
        logger.error(f"No valid models found to train. Available in config: {[m.name for m in config.models]}")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default=str(DEFAULT_CONFIG_PATH), help="Path to config file")
