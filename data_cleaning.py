...
def validate_dataset(config: PipelineConfig) -> bool:
    ...
    for split_name, split_path in splits.items():
        ...
        if labels_dir.exists():
            label_files = {
                f.name for f in labels_dir.iterdir() if f.is_file() and f.suffix == ".txt"
            }
        else:
            label_files = set()
        ...
        for img_path in progress_bar(images, desc=f"{split_name} images"):
            ...
            try:
                ...
                with Image.open(img_path) as img:
                    ...
                    if w != img_size or h != img_size:
                        warnings.append(
                            f"Image {img_path.name} is {w}x{h}, expected {img_size}x{img_size}"
                        )
            ...
            for line_idx, line in enumerate(lines):
                ...
                if any(c < 0.0 or c > 1.0 for c in coords):
                    errors.append(
                        f"Coordinates outside [0, 1] in {label_path.name} line {line_idx+1}"
                    )
...
def _save_plot(fig: plt.Figure, name: str, viz_dir: Path) -> None:
    ...
    fig.savefig(
        viz_dir / f"{name}.png", dpi=300, bbox_inches="tight", facecolor=fig.get_facecolor()
    )
    fig.savefig(
        viz_dir / f"{name}.svg", format="svg", bbox_inches="tight", facecolor=fig.get_facecolor()
    )
    plt.close(fig)


def _generate_visualizations(
    report: dict[str, Any], heatmap_grid: np.ndarray, viz_dir: Path, names: list[str]
) -> None:
    ...
    sns.barplot(
        ...
        x=counts,
        hue=classes,
        palette=palette[: len(classes)] if len(classes) <= len(palette) else "viridis",
        legend=False,
        ax=ax,
    )
    ...
    fig, ax = plt.subplots(figsize=(12, 8))
    sns.histplot(
        report["objects_per_image"],
        bins=range(0, max(report["objects_per_image"]) + 2) if report["objects_per_image"] else 10,
        color="#00b4d8",
        ax=ax,
        discrete=True,
    )
    ...
    if np.sum(heatmap_grid) > 0:
        ...
        sns.heatmap(
            heatmap_smoothed, cmap="magma", xticklabels=False, yticklabels=False, ax=ax, cbar=True
        )
