...
def _save_figure(fig: plt.Figure, name: str, output_dir: Path) -> None:
    ...
    fig.savefig(
        output_dir / f"{name}.png", dpi=300, bbox_inches="tight", facecolor=fig.get_facecolor()
    )
    fig.savefig(
...
def generate_dataset_plots(config: PipelineConfig) -> None:
    ...
    if objs:
        ...
        sns.histplot(
            objs,
            bins=range(0, max(objs) + 2) if max(objs) < 50 else 30,
            color="#00b4d8",
            ax=ax,
            discrete=max(objs) < 50,
        )
...
def generate_training_plots(config: PipelineConfig) -> None:
            ...
            try:
                ...
                for col in metric_cols:
                    if col in df.columns:
                        ax.plot(
                            df.index, df[col], label=f"{model_name}", color=palette[i], linewidth=2
                        )
                        plotted = True
                        break
    ...
    # Metrics
    plot_metric(
        ["metrics/mAP50-95(B)", "metrics/mAP50-95"], "Validation mAP50-95", "mAP", "train_map50_95"
    )
...
def generate_comparison_plots(config: PipelineConfig) -> None:
    ...
    fig, ax = plt.subplots(figsize=(12, 8))
    ax.scatter(
        df["FPS"], df["mAP50-95"], s=df["Size_MB"] * 5, alpha=0.7, c=range(len(df)), cmap="viridis"
    )

    for i, txt in enumerate(df["Model"]):
        ax.annotate(
            txt,
            (df["FPS"].iloc[i], df["mAP50-95"].iloc[i]),
            xytext=(10, 10),
    ...
    for m in ["mAP50-95", "F1", "FPS"]:
        radar_df[f"norm_{m}"] = (radar_df[m] - radar_df[m].min()) / (
            radar_df[m].max() - radar_df[m].min() + 1e-10
        )
    for m in ["Size_MB", "Latency_ms"]:
        radar_df[f"norm_{m}"] = 1.0 - (radar_df[m] - radar_df[m].min()) / (
            radar_df[m].max() - radar_df[m].min() + 1e-10
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type",
        type=str,
        choices=["all", "dataset", "training", "evaluation", "comparison"],
        default="all",
    )
