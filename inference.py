...
def detect_violations(detections: list[dict[str, Any]]) -> tuple[int, list[str]]:
    ...
    ppe_items = [d for d in detections if d["class_name"] in ["helmet", "vest", "goggles", "boots", "gloves"]]
...
def annotate_frame(
    ...
    if violations:
        y_offset = 60
        cv2.putText(annotated, f"VIOLATIONS: {len(violations)}", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        for v in violations:
            y_offset += 25
            cv2.putText(annotated, v, (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
...
def run_inference(
    ...
    if isinstance(source, list):
        if len(source) == 1:
            ...
            for img_path in source:
                res = _process_image(model, img_path, conf_thresh, iou_thresh, config, save, show, img_out_dir, logger)
    ...
    # Detect source type
    if source_str.isdigit() or source_str.startswith("rtsp://") or ".mp4" in source_str.lower() or "youtube" in source_str.lower() or "youtu.be" in source_str.lower():
        _process_video_stream(model, source_str, conf_thresh, iou_thresh, config, save_video, show, out_dir, logger)
    else:
        ...
        if path.is_dir():
            ...
            for ext in ["*.jpg", "*.jpeg", "*.png"]:
                for img_path in progress_bar(list(path.glob(ext)), desc="Processing folder"):
                    res = _process_image(model, str(img_path), conf_thresh, iou_thresh, config, save, False, img_out_dir, logger)
                    all_results.append(res)
            _save_reports(all_results, out_dir, logger)
        else:
            res = _process_image(model, source_str, conf_thresh, iou_thresh, config, save, show, img_out_dir, logger)
            _save_reports([res], out_dir, logger)


def _process_image(model: YOLO, img_path: str, conf: float, iou: float, config: PipelineConfig, save: bool, show: bool, out_dir: Path, logger) -> dict:
...
def _process_video_stream(model: YOLO, source_str: str, conf: float, iou: float, config: PipelineConfig, save_video: bool, show: bool, out_dir: Path, logger) -> None:
    ...
    try:
        while True:
            ...
            results = model.predict(frame, imgsz=config.dataset.img_size, conf=conf, iou=iou, verbose=False)
            result = results[0]
